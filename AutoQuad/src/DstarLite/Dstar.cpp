...
void mpCallback(const std_msgs::Int32::ConstPtr& msg) {
	mission_planner = msg->data;
}

void xobsCallback(const std_msgs::Float64::ConstPtr& msg) {
	x_obs = msg->data;
}

void yobsCallback(const std_msgs::Float64::ConstPtr& msg) {
	y_obs = msg->data;
}

void robsCallback(const std_msgs::Float64::ConstPtr& msg) {
	r_obs = msg->data;
}

void NodeIDCallback(const std_msgs::Int32::ConstPtr& msg) {
	NodeID = msg->data;
}
...
int main(int argc, char **argv)
{
  ...
  //DECLARE TOPICS TO PUBLISH
  ros::Publisher mission_planner_pub = n.advertise<std_msgs::Int32>("mission_planner", 1);
  
  //SUBSCRIBE TO NODES
  ros::Subscriber sub = n.subscribe("testInt",100, testCallback);
  ros::Subscriber sub1 = n.subscribe("mission_planner",100,mpCallback);
  ros::Subscriber sub2 = n.subscribe("/obstacle/x_obs",100,xobsCallback);
  ros::Subscriber sub3 = n.subscribe("/obstacle/y_obs",100,yobsCallback);
  ros::Subscriber sub4 = n.subscribe("/obstacle/r_obs",100,robsCallback);
  ros::Subscriber sub5 = n.subscribe("/control/NodeID",100,NodeIDCallback);

  ros::Rate loop_rate(30);


  //Inialize shit for D*Lite
  int replan_counter = 0;
  //we want to find a path that goes from here to here
  int startNodeIndex = 20; //subtract 1 from desired 
  int goalNodeIndex = 3310; //subtract 1 from desired

  saveBlankEdgeFile("/home/ubuntu/.ros/edges_blank.txt", "/home/ubuntu/.ros/edges.txt"); //makes a copy of blank edge file that we will update with addition of obstacles later

  //create graph
  Graph G; 
  G.readGraphFromFiles("/home/ubuntu/.ros/nodes_blank.txt", "/home/ubuntu/.ros/edges_blank.txt");
  ...
  //initialize heap
  Heap<Node> H(100); // this is the heap (starts with space for 100 items
  ...
  printf("Start and end nodes for this flight are: ");
  cout << startNodeIndex;
  cout << endl;
  cout << goalNodeIndex;
  cout << endl;
  printf("PREPPED FOR FLIGHT. WAITING FOR MISSION_PLANNER=7 FOR MOTION PLANNING\n");
  ...
  while (ros::ok())
  {
	...
	if (mission_planner == 7) {
		//run the shit for first solution
		computeShortestPath(G,H,startNode,goalNode,startNodeIndex,goalNodeIndex,km);
		G.saveSearchTreeToFile("/home/ubuntu/.ros/search_tree1.txt");
		G.savePathToFile("/home/ubuntu/.ros/output_path1.txt", startNode);
		
		//publish 2 to mission planner
		mission_planner = 2;
		std_msgs::Int32 pub_mission_planner;
		pub_mission_planner.data = mission_planner;
		mission_planner_pub.publish(pub_mission_planner);
	}
	
	
	//New obstacle detected
	if (mission_planner == 6) {
		//move the start node to the next position along the solution path
  		Node* startNode = &G.nodes[NodeID];
  		Node* goalNode = &G.nodes[goalNodeIndex];
		...
		km = km + h;
		lastNode = startNode;
		updateEdgeCostsandNodes(G, H, startNode, goalNode, km, "/home/ubuntu/.ros/nodes_blank.txt", "/home/ubuntu/.ros/edges.txt", x_obs, y_obs, r_obs); //note: use the other edge file as this one gets modified
		...
		if (replan_counter == 1) {
		    G.saveSearchTreeToFile("/home/ubuntu/.ros/search_tree2.txt");
		    G.savePathToFile("/home/ubuntu/.ros/output_path2.txt", startNode);
		    //publish 3 to mission planner
		    mission_planner = 3;
		    std_msgs::Int32 pub_mission_planner;
		    pub_mission_planner.data = mission_planner;
		    mission_planner_pub.publish(pub_mission_planner);
		...
		if (replan_counter == 2) {
			G.saveSearchTreeToFile("/home/ubuntu/.ros/search_tree3.txt");
			G.savePathToFile("/home/ubuntu/.ros/output_path3.txt", startNode);
			//publish 4 to mission planner
		    mission_planner = 4;
		    std_msgs::Int32 pub_mission_planner;
		    pub_mission_planner.data = mission_planner;
		    mission_planner_pub.publish(pub_mission_planner);
		...
		if (replan_counter == 3) {
			G.saveSearchTreeToFile("/home/ubuntu/.ros/search_tree4.txt");
			G.savePathToFile("/home/ubuntu/.ros/output_path4.txt", startNode);
			//publish 5 to mission planner
		    mission_planner = 5;
		    std_msgs::Int32 pub_mission_planner;
		    pub_mission_planner.data = mission_planner;
		    mission_planner_pub.publish(pub_mission_planner);
